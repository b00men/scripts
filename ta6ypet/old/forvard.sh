...
# Redhats or Fedoras init.d script if you prefer to use it.
...
# ip_conntrack, and some of the optional modules, Ive left these
...
# Its used to build a DMZ.  Since thats not a focus of this firewall
# script, its not enabled by default, but is included for reference.
...
# are generally considered a security risk and shouldnt be
...
# However, well ensure the secure_redirects option is on instead.
...
# a syn packet.  If it doesnt, it is likely a
...
# Its applied to INPUT on the external or Internet interface.
...
# If you dont query-source the server to port 53 and you have problems,
...
# Game servers (COD4,teeworlds)
$IPT -A udp_inbound -p UDP -s 0/0 --destination-port 28960:28962 -j ACCEPT
$IPT -A udp_inbound -p UDP -s 0/0 --destination-port 8303 -j ACCEPT
$IPT -A udp_inbound -p UDP -s 0/0 --destination-port 8304 -j ACCEPT
# Proxy
$IPT -A udp_inbound -p UDP -s 0/0 --destination-port 8080 -j ACCEPT
# SVN
$IPT -A udp_inbound -p UDP -s 0/0 --destination-port 3690 -j ACCEPT
# HTTPS
$IPT -A udp_inbound -p UDP -s 0/0 --destination-port 443 -j ACCEPT
...
# Its applied on INPUT from the external or Internet interface.
...
# User specified allowed UDP protocol
# Game servers (COD4,teeworlds)
$IPT -A tcp_inbound -p TCP -s 0/0 --destination-port 28960:28962 -j ACCEPT
$IPT -A tcp_inbound -p TCP -s 0/0 --destination-port 8303 -j ACCEPT
$IPT -A tcp_inbound -p TCP -s 0/0 --destination-port 8304 -j ACCEPT
# Proxy
$IPT -A tcp_inbound -p TCP -s 0/0 --destination-port 8080 -j ACCEPT
# SVN
$IPT -A tcp_inbound -p TCP -s 0/0 --destination-port 3690 -j ACCEPT
# HTTPS
...
# if they arent accepted elsewhere, it will only ensure that
...
# Log packets that still dont match
